...
func deterministicOracleFixture() *Index {
	...
	acmeModel := add(fixtureprojProject, "model", uint32(os.ModeDir))
	acmeTrainingdata := add(acmeModel, "trainingdata", uint32(os.ModeDir))
	acmeRaw := add(acmeTrainingdata, "raw files", uint32(os.ModeDir))
	add(acmeRaw, "volume-000.nrrd", 0)

...
- **Multi-term posting intersection (`completeMultiTermNameGramCandidates`)**.
  Multi-word name queries such as `aker log`, `acme log`, and `discovery log`
...
- **CLI searches prefer the resident service**. A configured `dbs` list in
  ...
  `seekfs search acmepy` dropped from ~15 s to ~300 ms; `--under` scoped
  searches to ~30 ms.
...
- Multi-term name queries (e.g. `aker log`, `acme log`, `discovery log`,
...
(was ~30 s), `aker log` / `acme log` / `discovery log` / `exxon log` all
...
counts 8068 files in well under a second); `seekfs search acmepy` answers in
